...
module InTheZone
  ...
  DefaultOptions = { classes: "aa-nthzn", format: "%a, %d %b %Y %T %z" }.freeze
  TagTemplate = "<span class=\"<%= classes %>\" data-timestamp=\"<%= timestamp %>\"><%= time_string %></span>".freeze

  def time_tag( time=Time.now.utc, opts={} )
    tag_data = normalize_opts( opts )
    tag_template.result( tag_data.merge( time_string: time_string( time.utc, tag_data[ :format ] ), timestamp: time.utc.to_i ) )
  end

  def date_tag( date, opts={} )
    time_tag( date.to_time, opts.merge( date_only: true ) )
  end
